...
The compiler pipeline mirrors a classic back end. A minimal front end
(`IrPlan::parse`) accepts a line-based plan source — `input N`, `copy`,
`add_scalar V`, `elementwise K`, `matmul K M K N`, `matmul_act K M K N A`,
with `#` comments and line-numbered typed errors — and produces the
source-independent IR. `IrPlan::lower` is the emitter: it resolves kernel
names to registry indices, validates each kernel's declared operation kind,
tracks chain lengths through shape changes, and emits the fixed-width
records that `compile_plan` digests. `IrPlan::certify` is behavioral
certification: it executes the lowered plan through the full runtime and
compares every output element bitwise against direct reference evaluation.
`artifact create-plan` ties it together: source to IR to records to an
artifact whose provenance section carries the plan's UOR address, published
atomically (temporary file, validation, rename) so partial artifacts are
never visible. The record format is deliberately a tiny fixed instruction
set with immediates carried inline; fusion decisions (a fused `MATMUL_ACT`
op versus separate matmul and activation ops) are the compiler's choice,
not the runtime's.
