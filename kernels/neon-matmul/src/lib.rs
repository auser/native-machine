...
//!
//! On AArch64 the kernel uses a 4x4 register-blocked NEON micro-kernel: four
//! rows of `C` are accumulated in vector registers while one row segment of
//! `B` is loaded per inner step, vectorizing across columns instead of
//! reducing along `K`. This uses fused multiply-add, so results may differ
//! from the scalar reference in the last mantissa bit; the reference kernel
//! remains the oracle. Other targets use the scalar fallback. The kernel
//! performs no heap allocation, hidden copies, or layout conversion.
...
fn matmul_neon(a: &[f32], b: &[f32], output: &mut [f32], m: usize, k: usize, n: usize) {
    ...
    // lanes starting at inner * n + column with column + 4 <= n and inner < k,
    // staying within b.len() == k * n; every store writes four lanes within
    // output.len() == m * n; every `get_unchecked` on `a` stays below
    // a.len() == m * k because row < m and inner < k. NEON accesses do not
    // require alignment.
    unsafe {
        let mut row = 0;
        while row + 4 <= m {
