//! AVX2 add-one kernel implementing the Native Machine ABI v3.
//!
//! Contract: `output[i] = input[i] + 1.0` over contiguous `f32` buffers,
//! identical to `reference-add-one`. On x86_64 hosts with AVX2 (detected at
//! runtime) the hot loop uses four-way unrolled AVX2 vector operations; other
//! hosts use the scalar fallback. The kernel performs no heap allocation and
//! is deterministic.
...
/// `output[i] = input[i] + 1.0`, AVX2-accelerated on x86_64 when available.
pub fn add_one(input: &[f32], output: &mut [f32]) {
    #[cfg(target_arch = "x86_64")]
    if std::arch::is_x86_feature_detected!("avx2") {
        // SAFETY: AVX2 support was detected at runtime on this host.
        unsafe { add_one_avx2(input, output) };
        return;
    }
...
unsafe fn add_one_avx2(input: &[f32], output: &mut [f32]) {
    use std::arch::x86_64::{_mm256_add_ps, _mm256_loadu_ps, _mm256_set1_ps, _mm256_storeu_ps};
    // SAFETY: AVX2 support was confirmed by runtime detection at the call
    // site, so the target-feature contract of every intrinsic used here is
    // satisfied. All loads and stores stay within the first `vectorized`
    // elements, which is <= input.len() == output.len(); unaligned AVX2
    // accesses do not require alignment.
    unsafe {
        ...
        while offset < vectorized {
            ...
            _mm256_storeu_ps(output.as_mut_ptr().add(offset), _mm256_add_ps(first, ones));
            _mm256_storeu_ps(
                output.as_mut_ptr().add(offset + 8),
                _mm256_add_ps(second, ones),
            );
            _mm256_storeu_ps(
                output.as_mut_ptr().add(offset + 16),
                _mm256_add_ps(third, ones),
            );
            _mm256_storeu_ps(
                output.as_mut_ptr().add(offset + 24),
                _mm256_add_ps(fourth, ones),
            );
            offset += 32;
        }
...
static NAME: &[u8] = b"avx2-add-one\0";
