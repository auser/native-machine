//! Reference ReLU kernel implementing the Native Machine ABI v3.
//!
...
/// Scalar reference implementation: `output[i] = max(input[i], 0.0)`.
...
static NAME: &[u8] = b"reference-relu\0";
...
mod tests {
    ...
    #[test]
    fn relu_matches_scalar_oracle() {
        let input = [-2.5_f32, -0.0, 0.0, 1.5, 1e30];
        let mut output = [0.0_f32; 5];
        relu(&input, &mut output);
        for (source, destination) in input.iter().zip(output) {
            assert_eq!(source.max(0.0), destination);
        }
